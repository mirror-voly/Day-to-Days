//
//  Constraints.swift
//  Day to Days
//
//  Created by mix on 08.09.2024.
//

import Foundation

enum Constraints {
    static let buttonSize: CGFloat = 40
    static let sliderCircleSmall: CGFloat = 15
    static let sliderCircleBig: CGFloat = 30
    static let eventDateTableSize: CGFloat = 100
    static let eventsItemViewCicleSize: CGFloat = 25
    static let eventsItemViewCicleHoleSize: CGFloat = 10
    static let eventsItemViewDateFrameSize: CGFloat = 60
    static let dateTextMinimumScaleFactor: CGFloat = 0.6
	
    static let cornerRadius: CGFloat = 20
    static let shadowRadius: CGFloat = 10
	static let primaryOpacity: CGFloat = 0.8
	static let originalSize: CGFloat = 1
	
    static let emptyViewPaddingToTheTop: CGFloat = -60
    static let emptyViewFrameSize: CGFloat = 200
	
    static let rotationAngle: CGFloat = 180
    static let animationDuration: CGFloat = 3
	static let listItemAnimationScale: CGFloat = 0.5
	static let scaleColorItem: CGFloat = 0.2
	static let frameBoundsLimit: CGFloat = 70
	
    static let dividerWidth: CGFloat = 125
    static let containerRelativeFrame: CGFloat = 11
    static let clearableTextFieldHeight: CGFloat = 25
    static let dragGestureDistance: CGFloat = 100
    static let eventsItemViewHeight: CGFloat = 115
	
	static let widgetNumberFontSize: CGFloat = 75
	static let widgetStackSpaser: CGFloat = 5
	
    static let notificationDateWidth: CGFloat = 80
    static let notificationSetupViewHeight: CGFloat = 250
    static let notificationSmallFontSize: CGFloat = 15
    static let notificationMediumFontSize: CGFloat = 25
    static let notificationBigFontSize: CGFloat = 55
	
	static let scalePower: CGFloat = 2
	static let scaleEffect: CGFloat = 1.1
	
	static let renderScale: CGFloat = 2
	static let renderedImageSize: CGFloat = 300
	static let shareCentralPartSize: CGFloat = 120
	static let shareCentralInageFontSize: CGFloat = 50
	static let shareHorizontalPadding: CGFloat = 6
}
